...
- [uv](https://docs.astral.sh/uv/) (パッケージ/Python 管理は uv に統一)
...
Python のバージョンは `.python-version` で固定されており、`uv` が必要に応じて自動でダウンロードします。

## セットアップ

```bash
# 依存と仮想環境を一括で用意 (.venv が自動生成される)
uv sync
```

`uv.lock` を使って再現性のあるインストールが行われます。明示的に `source .venv/bin/activate` する必要はなく、後述の `uv run` 経由でそのまま実行できます。
...
```bash
uv run python send_signal.py Alice
uv run python send_signal.py Bob
uv run python send_signal.py Carol
```
...
```bash
uv run python send_query.py greetings   # これまで作られた挨拶文のリスト
uv run python send_query.py pending     # まだ処理待ちの名前 (基本は空のはず)
```
...
```bash
uv run python send_signal.py exit
```
...
| `pyproject.toml` / `uv.lock` | 依存関係 (`temporalio`) と uv によるロック |
